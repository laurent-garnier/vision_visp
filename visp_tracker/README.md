...
This package can be compiled like any other colcon package using `colcon build`. 

### Prerequisities

First you need to install ViSP as a system dependency. This can be achived using `ros-rolling-visp` package for Ubuntu. Just run:
...
If the package is not available (this is for example the case for Fedora) or if you want to use a more recent version of ViSP, you can also install ViSP from source:
	...
	$ cd ~
 	$ git checkout https://github.com/lagadic/visp

Add follow the INSTALL.txt instructions.
