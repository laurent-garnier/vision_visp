  ...
  if (cmd.using_hinkley()) {
    if (cmd.get_verbose())
      RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"),
                         "Initialising hinkley with alpha=" << cmd.get_hinkley_alpha()
                                                            << " and delta=" << cmd.get_hinkley_delta());
...
vpDetectorBase &Tracker_::get_detector() { return *detector_; }
      ...
      for (size_t i = 0; i < detector_->getNbObjects(); i++) {
        if (detector_->getMessage(i) == cmd.get_code_message()) {
          ...
          RCLCPP_WARN_STREAM(rclcpp::get_logger("rclcpp"),
                             "Code with message \"" << cmd.get_code_message() << "\" found");
          return true;
        }
      }
      RCLCPP_WARN_STREAM(rclcpp::get_logger("rclcpp"),
                         "Code with message \"" << cmd.get_code_message() << "\" not found");
      return false;
    }
  }
  return false;
...
bool Tracker_::model_detected(msm::front::none const &)
{
  ...
  for (unsigned int i = 0; i < 4; i++) {
    ...
    if (cmd.get_verbose()) {
      RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "model inner corner: (" << model_inner_corner[i].get_i() << ","
  ...
  try {
    ...
    tracker_->setCameraParameters(cam_);
    {
      ...
      if (cam.get_px() != 558)
        RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "detection Camera parameters: \n" << cam_);
    }
...
void Tracker_::updateMovingEdgeSites(visp_tracker::msg::MovingEdgeSites &sites)
{
