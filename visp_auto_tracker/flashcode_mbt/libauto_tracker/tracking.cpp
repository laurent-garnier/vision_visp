    ...
    flush_display_(flush_display)
{
  ...
  if (cmd.using_hinkley()) {
    if (cmd.get_verbose())
      std::cout << "Initialising hinkley with alpha=" << cmd.get_hinkley_alpha()
                << " and delta=" << cmd.get_hinkley_delta() << std::endl;
  ...
  tracker_->loadModel(cmd.get_mbt_cad_file());  // load the 3d model, to read .wrl model the 3d party library coin is
                                                // required, if coin is not installed .cao file can be used.
...
vpMbTracker &Tracker_::get_mbt() { return *tracker_; }

std::vector<vpPoint> &Tracker_::get_points3D_inner() { return points3D_inner_; }

std::vector<vpPoint> &Tracker_::get_points3D_outer() { return points3D_outer_; }

std::vector<vpPoint> &Tracker_::get_points3D_middle() { return points3D_middle_; }

std::vector<vpPoint> &Tracker_::get_flashcode() { return f_; }

vpImage<vpRGBa> &Tracker_::get_I() { return *I_; }

vpCameraParameters &Tracker_::get_cam() { return cam_; }

CmdLine &Tracker_::get_cmd() { return cmd; }

template <> const cv::Rect &Tracker_::get_tracking_box<cv::Rect>() { return cvTrackingBox_; }

template <> const vpRect &Tracker_::get_tracking_box<vpRect>() { return vpTrackingBox_; }

bool Tracker_::input_selected(input_ready const &evt) { return vpDisplay::getClick(evt.I, false); }

bool Tracker_::no_input_selected(input_ready const &evt) { return !input_selected(evt); }
...
bool Tracker_::flashcode_redetected(input_ready const &evt)
{
  ...
  if (cvTrackingBox_init_) {
    ...
    double timeout = cmd.get_dmx_timeout() *
                     (double)(get_tracking_box<cv::Rect>().width * get_tracking_box<cv::Rect>().height) /
                     (double)(bgr.cols * bgr.rows);
    return detector_->detect(subImage, (unsigned int)timeout, get_tracking_box<cv::Rect>().x,
                             get_tracking_box<cv::Rect>().y);
  } else {
    return detector_->detect(bgr, cmd.get_dmx_timeout(), 0, 0);
  }
#else
...
bool Tracker_::model_detected(msm::front::none const &)
{
  ...
  for (unsigned int i = 0; i < 4; i++) {
    ...
    vpMeterPixelConversion::convertPoint(cam_, points3D_outer_[i].get_x(), points3D_outer_[i].get_y(),
                                         model_outer_corner[i]);
    vpMeterPixelConversion::convertPoint(cam_, points3D_inner_[i].get_x(), points3D_inner_[i].get_y(),
                                         model_inner_corner[i]);

    if (cmd.get_verbose()) {
      std::cout << "model inner corner: (" << model_inner_corner[i].get_i() << "," << model_inner_corner[i].get_j()
                << ")" << std::endl;
    }
  }
  ...
  try {
    ...
    tracker_->setCameraParameters(cam_);
    {
      ...
      if (cam.get_px() != 558)
        ROS_INFO_STREAM("detection Camera parameters: \n" << cam_);
    }
  ...
  try {
    ...
    if (cmd.using_adhoc_recovery() || cmd.log_checkpoints()) {
      for (unsigned int p = 0; p < points3D_middle_.size(); p++) {
        ...
        vpMeterPixelConversion::convertPoint(cam_, points3D_inner_[p].get_x(), points3D_inner_[p].get_y(), _u_inner,
                                             _v_inner);
        ...
        int u = (int)_u;
        int v = (int)_v;
        for (int i = std::max(u - region_width, 0); i < std::min(u + region_width, (int)evt.I.getWidth()); i++) {
          for (int j = std::max(v - region_height, 0); j < std::min(v + region_height, (int)evt.I.getHeight()); j++) {
            acc(Igray_[j][i]);
...
void Tracker_::track_model(input_ready const &evt)
{
  ...
  for (unsigned int i = 0; i < points3D_outer_.size(); i++) {
    ...
    vpMeterPixelConversion::convertPoint(cam_, points3D_inner_[i].get_x(), points3D_inner_[i].get_y(), u_inner,
                                         v_inner);
...
Tracker_::statistics_t &Tracker_::get_statistics() { return statistics; }

void Tracker_::set_flush_display(bool val) { flush_display_ = val; }

bool Tracker_::get_flush_display() { return flush_display_; }

void Tracker_::updateMovingEdgeSites(visp_tracker::MovingEdgeSitesPtr sites)
{
  if (!sites)
    return;
...
void Tracker_::updateKltPoints(visp_tracker::KltPointsPtr klt)
{
  if (!klt)
    return;
